...
def collect_item(items_list, i):
    if not i in items_list:  # if the function is used this row will looks like that: if not is_item_list(items_list, i):
        items_list.append(i)
    return items_list


def drop_item(items_list, i):
    if i in items_list:  # if the function is used this row will looks like that: if is_item_list(items_list, i):
        items_list.remove(i)
    return items_list
...
def combine_items(items_list, i):
    old_item = i.split(":")[0]
    new_item = i.split(":")[1]
    if old_item in items_list:  # if the function is used this row will looks like that: if is_item_list(items_list, old_item):
