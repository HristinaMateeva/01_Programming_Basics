...
class Storage:
    ...
    def delete_category(self, category_id):
        ...
        # self.categories.remove(next(filter(lambda c: c.id == category_id, self.categories))) the same as the row above

    def delete_topic(self, topic_id):
        self.topics.remove([t for t in self.topics if t.id == topic_id][0])
        # self.topics.remove(next(filter(lambda t: t.id == topic_id, self.topics))) the same as the row above

    def delete_document(self, document_id):
        self.documents.remove([d for d in self.documents if d.id == document_id][0])
        # self.documents.remove(next(filter(lambda d: d.id == document_id, self.documents))) the same as the row above
